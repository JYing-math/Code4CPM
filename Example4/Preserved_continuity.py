...
def get_boundary_points(num):
    ...
    xb2 = torch.cat((index2, index2, torch.full_like(index1, -1)), dim=1)
    xb3 = torch.cat((index1, torch.ones_like(index1), index2), dim=1)
    xb4 = torch.cat((index1, torch.full_like(index1, -1), index2), dim=1)
    xb5 = torch.cat((torch.ones_like(index1), index1, index2), dim=1)
    xb6 = torch.cat((torch.full_like(index2, -1), index1, index2), dim=1)
...
def level_set_fuction(x):
    ...
    x2 = x[:,1]
    x3 = x[:,2]
    lf = torch.pow(x1, 2) + torch.pow(x2, 2) + torch.pow(x3, 2) - 0.5 ** 2
...
def u_x(x):
    ...
    x1 = x[:, 0]
    u1 = x1 * (-1 + torch.exp(0.25 - r2))
    u2 = -1 + torch.cos(0.25 - r2)
...
def f_x(x):
    ...
    x1 = x[:, 0]
    r2 = torch.sum(torch.pow(x, 2), 1)
    f1 = x1 * (4 * r2 - 10) * torch.exp(0.25 - r2)
    f2 = -4 * r2 * torch.cos(0.25 - r2) + 6 * torch.sin(0.25 - r2)
...
def psi_x(x):
    ...
    u2 = torch.unsqueeze(u2, 1)
    du1 = autograd.grad(outputs=u1, inputs=x,
                        grad_outputs=torch.ones_like(u1),
                        create_graph=True, retain_graph=True, only_inputs=True)[0]
    du2 = autograd.grad(outputs=u2, inputs=x,
...
def func_loss_op(func_params, x_op, f_op):
    ...
    phi_zz = 2. * torch.ones_like(phi_z)

    U_xx = u_xx + u_xh * phi_x + u_xh * phi_x + u_hh * phi_x * phi_x + u_h * phi_xx
    U_yy = u_yy + u_yh * phi_y + u_yh * phi_y + u_hh * phi_y * phi_y + u_h * phi_yy
    U_zz = u_zz + u_zh * phi_z + u_zh * phi_z + u_hh * phi_z * phi_z + u_h * phi_zz
    loss_op = U_xx + U_yy + U_zz - f_op
    return loss_op
...
def func_loss_if(func_params, x_if, nor, psi):
    def f(x, func_params):
        fx = func_model(func_params, x)
        return fx.squeeze(0).squeeze(0)
    ...
    psi2 = psi[1]
    psi3 = psi[2]
    loss_if = (u_h * phi_x - psi1) * nor1 + (u_h * phi_y - psi2) * nor2 + (u_h * phi_z - psi3) * nor3
...
def train_PINNs_LM(func_params, LM_setup, tr_input, lossval, lossval_dbg):
    ...
    [X_op, F_op, X_on, F_on, X_b, F_b, X_if, F_if, nor, NL, NL_sqrt] = tr_input #训练参数
    ...
    try:
        while (lossval[-1] > tol_main) and (step <= tr_iter_max):
            ...
            if (Comput_old == True):  # need to compute loss_old and J_old
                ...
                Lif = vmap((func_loss_if), (None, 0, 0, 0))(func_params, X_if, nor, F_if).flatten().detach()
            ...
            if criterion:
                ...
                per_sample_grads = vmap(jacrev(func_loss_if), (None, 0, 0, 0))(func_params, X_if, nor, F_if)
                cnt = 0
                for g in per_sample_grads:
                    g = g.detach()
...
def generate_data(num_omega, num_b, num_if):
    ...
    len_xop = len(xop)
    len_xon = len(xon)
    len_sum = num_omega + 6 * num_b + num_if

    NL = [len_sum, len_xop, len_xon, 6 * num_b, num_if]
    NL_sqrt = np.sqrt(NL)
    ...
    return xop_tr, fop_tr, xon_tr, fon_tr, xb_tr, fb_tr, xif_tr, fif_tr, nor, NL, NL_sqrt
# Essential namedtuples in the model
DataInput = namedtuple( "DataInput" , [ "X_op" , "F_op" , "X_on" , "F_on", "X_b" , "F_b" , "X_if" , "F_if", "nor", "NL" , "NL_sqrt"] )
...
for char in char_id:
    ...
    xop_tr, fop_tr, xon_tr, fon_tr, xb_tr, fb_tr, xif_tr, fif_tr, nor, NL_tr, NL_sqrt_tr = generate_data(num_omega, num_b, num_if)
    tr_input = DataInput(X_op=xop_tr, F_op=fop_tr, X_on=xon_tr, F_on=fon_tr, X_b=xb_tr, F_b=fb_tr, X_if=xif_tr, F_if=fif_tr, nor=nor, NL=NL_tr, NL_sqrt=NL_sqrt_tr)
