...
def get_omega_points(num):
    ...
    xo1 = torch.cat((x1, x2), dim=1)
    x1 = 0.4*(2 * torch.rand(300, 1) - 1)
    x2 = 0.4*(2 * torch.rand(300, 1) - 1)
    xo2 = torch.cat((x1, x2), dim=1)
    x = torch.cat((xo1, xo2), dim=0)
    return x
...
def u_x(x):
    ...
    index = m
    r = torch.sqrt(torch.sum(torch.pow(x, 2), 1))
    u1 = torch.pow(r, 3)/1000 - (1/1000 - 1.) * 0.5 ** 3
    u2 = torch.pow(r, 3)
...
def g_x(x):
    r = torch.sqrt(torch.sum(torch.pow(x, 2), 1))
    g = torch.pow(r, 3)/1000 - (1/1000 - 1.) * 0.5 ** 3
...
def func_loss_if(func_params, x_if, nor):
    ...
    phi_x = 4. * x_if[0]
    phi_y = 4. * x_if[1]
    loss_if = u_z * phi_x * nor1 + u_z * phi_y * nor2 - 3 * 0.25 * (1/1000 - 1)

    return np.sqrt(10)*loss_if
...
def generate_data(num_omega, num_b, num_if):
    ...
    plt.scatter(xo[:,0],xo[:,1],c='b')
    plt.scatter(xb[:,0],xb[:,1],c='g')
    plt.scatter(xif[:,0],xif[:,1],c='r')
    plt.title('Training points distribution')
    plt.savefig('points.jpg', bbox_inches='tight', dpi=600)
    ...
    len_xop = len(xop)
    len_xon = len(xon)
    len_sum = len_xop + len_xon + 4 * num_b + num_if
