...
class NeuralNet_Shallow(torch.nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim):
        ...
        nn.init.normal_(self.ln1.weight, mean=0, std=1.)
        nn.init.normal_(self.ln1.bias, mean=0, std=1.)
        nn.init.normal_(self.ln2.weight, mean=0, std=1.)
...
def func_loss_if(func_params, x_if, nor):
    ...
    phi_x = 4. * x_if[0]
    phi_y = 4. * x_if[1]
    loss_if = u_z * phi_x * nor1 + u_z * phi_y * nor2 + 4
...
'''-------------------------Error-------------------------'''
x1 = torch.linspace(-1, 1, 1000)
x2 = torch.linspace(-1, 1, 1000)
X, Y = torch.meshgrid(x1, x2)
point_r = torch.cat((Y.flatten()[:, None], Y.T.flatten()[:, None]), dim=1)
